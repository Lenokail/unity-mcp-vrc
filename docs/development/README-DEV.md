...
MCP for Unity organizes tools into **groups** (Core, Docs, VFX & Shaders, Animation, UI Toolkit, Scripting Extensions, Testing, ProBuilder, Profiling, VRChat/Udon). You can selectively enable or disable tools to control which capabilities are exposed to AI clients — reducing context window usage and focusing the AI on relevant tools.
